...
date = '20141114'
root_url = 'http://mitathletics.com/sports/m-baskbl/2014-15/boxscores/' + date + '_gmhh.xml?view='
...
# Persist a game's plays to the database
def persist_game_plays(events):

# Persist box score stats to the database by period
def persist_game_stats(stats_by_period):

# Persist starters by period to database
def persist_game_starters(starters_by_period):

# Process game metadata
def process_game_metadata(root_url):

# Persist game metadata
def persist_game_metadata(db, game_metadata):

# Process game summary
def process_game_summary(root_url):
	
# Persist game summary
def persist_game_summary(db, game_summary):

# Process game play by play
def process_game_play_by_play(root_url):

# Persist game play by play
def persist_game_play_by_play(db, game_play_by_play):

# Convert game play by play to detailed stats
def process_game_detailed_stats(game_play_by_play):

# Persists detailed game stats
def persist_game_detailed_stats(db, game_detailed_stats):

# Grand method for processing a game given the root urls
def process_game(root_url, db):
	# Handle game metadata
	game_metadata = process_game_metadata(root_url)
	persist_game_metadata(db, game_metadata)
	# Handle game starters and box scores
	game_summary = process_game_summary(root_url)
	persist_game_summary(db, game_summary)
	# Handle game play-by-play
	game_play_by_play = process_game_play_by_play(root_url)
	persist_game_play_by_play(game_play_by_play)
	# Turn game play-by-play into detailed stats
	game_detailed_stats = process_detailed_stats(game_play_by_play)
	persist_game_detailed_stats(db, game_detailed_stats)